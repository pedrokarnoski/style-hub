...
export function Header({ showBag = true }: { showBag: boolean }) {
  ...
  return (
    <div className="absolute z-20 w-full border-b bg-background lg:px-40">
      <div
        className={`flex h-16 items-center justify-between gap-6 px-6 ${!showBag ? 'justify-center' : ''}`}
      >
        <Link
          href="/"
          className={`flex items-center gap-3 font-semibold text-foreground ${!showBag ? 'absolute left-1/2 -translate-x-1/2 transform' : ''}`}
        >
